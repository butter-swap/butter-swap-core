...
describe('ButterPair', () => {
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount.add(1), 0, wallet.address, '0x', overrides)).to.be.revertedWith('Butter: K')
  ...
  it('feeTo:on', async () => {
    ...
    const token1Amount = expandTo18Decimals(1000)
    await addLiquidity(token0Amount, token1Amount)
    // 1000 0000 0000 0000 0000 00 A
    // 1000 0000 0000 0000 0000 00 B
    // 1000 0000 0000 0000 0000 00 LP
    const swapAmount = expandTo18Decimals(1)
    const expectedOutputAmount = bigNumberify('996006981039903216')
    await token1.transfer(pair.address, swapAmount)
    await pair.swap(expectedOutputAmount, 0, wallet.address, '0x', overrides)
    // SWAP 1 0000 0000 0000 0000 00 B for 9960 0698 1039 9032 16 A
    // LEFT 1001 0000 0000 0000 0000 00 B &  999 0039 9301 8960 0967 84 A
